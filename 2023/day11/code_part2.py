...
class Galaxy(Position):
    ...
    def __str__(self) -> str:
        # return str(self.idx) 
        return '{}: x:{} y:{}'.format(str(self.idx), str(self.x), str(self.y))
...
if __name__ == '__main__':
    ...
    universe = [[y for y in x] for x in lines]
    ...
    for x in universe:
        ...
        offsettedy = 0
        for y in x:
            # print('{},{}'.format(str(posx), str(posy)))
            if '#' not in [z[posy] for z in universe]:
                offsettedy += offset
            else:
                if y == '#':
                    galaxies.append(Galaxy(y, offsettedx, offsettedy, idx))
                    idx += 1
            posy += 1
            offsettedy += 1
        posx += 1
